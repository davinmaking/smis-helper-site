...
Pro is a flat **RM59 sekali bayar**. Shown in 3 places: promo strip, Pro card (`#proPrice`), pay modal (`#modalSub`). **Payment rail = Billplz (since 2026-07-03; ToyyibPay retired):** the on-site BM modal (name+emel, NO phone) POSTs `/api/license-purchase` with `slug:"smis-helper-pro"` + `returnUrl:"https://smis.davinhub.com/success"` (server-side allowlist) → Billplz FPX → back to our `success.html`, which polls `/api/license-purchase/status?ref=` for the key. The funnel stays entirely on this site — davinhub.com's zh store was removed 2026-09-14, so this is the only place SMIS Pro is sold. Merchant terms (EDUHELPER RESOURCES, refund policy — required by Billplz) live at `terma.html`; the pay modal and footer link to it.
